...
import { roomTypes, formatCurrency, hotelInfo } from '@/data/mockData';
...
const Booking = () => {
  ...
  const [checkOut, setCheckOut] = useState<Date | undefined>(
    searchParams.get('checkOut') ? new Date(searchParams.get('checkOut')!) : addDays(new Date(), 2)
  );
  ...
  const canProceed = () => {
    ...
    if (step === 'guest') return guestInfo.name && guestInfo.email && guestInfo.phone && guestInfo.idNumber;
    return true;
  };
            ...
            {steps.map((s, index) => (
              <div key={s.id} className="flex items-center">
                <div className={cn(
                  ...
                  steps.findIndex(st => st.id === step) > index ? 'bg-primary text-primary-foreground' :
                  'bg-muted text-muted-foreground'
                )}>
                  {steps.findIndex(st => st.id === step) > index ? <Check className="h-5 w-5" /> : index + 1}
                </div>
                <span className={cn(
                  ...
                  <CardContent className="space-y-6">
                    <div className="bg-muted/50 rounded-lg p-6 text-center">
                      <p className="text-muted-foreground mb-4">
                        Secure payment integration with Stripe requires Cloud activation.
                      </p>
                      <p className="text-sm text-muted-foreground mb-6">
                        For demo purposes, click "Complete Booking" to simulate a successful payment.
                      </p>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-6">
                    <div className="bg-muted rounded-lg p-6 text-center">
                      ...
                      <p className="text-3xl font-mono font-bold text-primary">{bookingId}</p>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        ...
                        <div>
                          ...
                          <p className="font-medium">{checkIn ? format(checkIn, 'EEEE, MMM dd, yyyy') : '-'}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Check-out</p>
                          <p className="font-medium">{checkOut ? format(checkOut, 'EEEE, MMM dd, yyyy') : '-'}</p>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <p className="font-medium text-primary">{formatCurrency(grandTotal)}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-muted/50 rounded-lg p-4">
                      <p className="text-sm text-muted-foreground">
                        A confirmation email has been sent to <strong>{guestInfo.email}</strong> with your booking details and invoice.
                      </p>
                    </div>
                    ...
                    {selectedRoom ? (
                      <>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">{hotelInfo.name}</p>
                        </div>
                        <Separator />
                        ...
                        <div className="space-y-2 text-sm">
                          <div className="flex justify-between">
                            <span className="text-muted-foreground">
                              {formatCurrency(selectedRoom.price)} x {nights} nights
                            </span>
